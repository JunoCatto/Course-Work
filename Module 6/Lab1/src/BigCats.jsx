...
function FilterButton({setCurrentCats}) {
    const filterByName = () => {
        setCurrentCats((currentCats) => [...currentCats].filter((cat) => cat.latinName.includes('Panthera')));
    }
...
export function BigCats() {
    ...
    return (
        <>
        {currentCats.map((cat) => (<SingleCat key={cat.name} name={cat.name} latinName={cat.latinName} />))}
